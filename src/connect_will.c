...
int mr_set_connect_will_values(packet_ctx *pctx, mr_connect_will_data *pwd) {
    if (mr_set_scalar(pctx, CONNECT_WILL_FLAG, pwd->will_flag)) return -1;
    if (mr_set_scalar(pctx, CONNECT_WILL_QOS, pwd->will_qos)) return -1;
    if (mr_set_scalar(pctx, CONNECT_WILL_RETAIN, pwd->will_retain)) return -1;
    if (mr_set_scalar(pctx, CONNECT_WILL_PROPERTY_LENGTH, 0)) return -1;
    if (mr_set_scalar(pctx, CONNECT_WILL_DELAY_INTERVAL, pwd->will_delay_interval)) return -1;
    if (mr_set_scalar(pctx, CONNECT_PAYLOAD_FORMAT_INDICATOR, pwd->payload_format_indicator)) return -1;
    if (mr_set_scalar(pctx, CONNECT_MESSAGE_EXPIRY_INTERVAL, pwd->message_expiry_interval)) return -1;

    if (mr_set_vector(pctx, CONNECT_CONTENT_TYPE, pwd->content_type, strlen(pwd->content_type) + 1)) return -1;
    if (mr_set_vector(pctx, CONNECT_RESPONSE_TOPIC, pwd->response_topic, strlen(pwd->response_topic) + 1)) return -1;
    if (mr_set_vector(pctx, CONNECT_CORRELATION_DATA, pwd->correlation_data, pwd->correlation_data_len)) return -1;
    if (mr_set_vector(pctx, CONNECT_WILL_USER_PROPERTIES, pwd->will_user_properties, pwd->will_user_properties_len)) return -1;
    if (mr_set_vector(pctx, CONNECT_WILL_TOPIC, pwd->will_topic, strlen(pwd->will_topic) + 1)) return -1;
    if (mr_set_vector(pctx, CONNECT_WILL_PAYLOAD, pwd->will_payload, pwd->will_payload_len)) return -1;
...
int mr_reset_connect_will_values(packet_ctx *pctx) {
    ...
    if (mr_reset_scalar(pctx, CONNECT_WILL_PROPERTY_LENGTH)) return -1;
...
int mr_get_connect_will_values(packet_ctx *pctx, mr_connect_will_data *pwd) {
    ...
    mr_get_u32(pctx, CONNECT_WILL_PROPERTY_LENGTH, &pwd->will_property_length, &exists);
...
int mr_validate_connect_will_data(packet_ctx *pctx, mr_connect_will_data *pwd) {
    if (mr_validate_will_qos(pwd)) return -1;
    if (mr_validate_will_retain(pwd)) return -1;
    if (mr_validate_will_property_length(pwd)) return -1;
    if (mr_validate_will_delay_interval(pwd)) return -1;
    if (mr_validate_payload_format_indicator(pwd)) return -1;
    if (mr_validate_message_expiry_interval(pwd)) return -1;
    if (mr_validate_content_type(pwd)) return -1;
    if (mr_validate_response_topic(pwd)) return -1;
    if (mr_validate_correlation_data(pwd)) return -1;
    if (mr_validate_will_user_properties(pwd)) return -1;
    if (mr_validate_will_topic(pwd)) return -1;
    if (mr_validate_will_payload(pwd)) return -1;

    return 0;
}

int mr_validate_connect_will_data_utf8(packet_ctx *pctx, mr_connect_will_data *pwd) {
    if (mr_validate_content_type_utf8(pwd)) return -1;
    if (mr_validate_response_topic_utf8(pwd)) return -1;
    if (mr_validate_will_user_properties_utf8(pwd)) return -1;
    if (mr_validate_will_topic_utf8(pwd)) return -1;
    if (mr_validate_will_payload_utf8(pwd)) return -1;
...
static int mr_validate_will_property_length(mr_connect_will_data *pwd) {
    if (!pwd->will_flag && pwd->will_property_length) {
        dzlog_error("will_flag false but will_property_length > 0");
        return -1;
    }
...
static int mr_validate_content_type(mr_connect_will_data *pwd) {
    if (!pwd->will_flag && pwd->content_type) {
...
static int mr_validate_response_topic(mr_connect_will_data *pwd) {
    if (!pwd->will_flag && pwd->response_topic) {
...
static int mr_validate_will_topic(mr_connect_will_data *pwd) {
    if (!pwd->will_flag && pwd->will_topic) {
