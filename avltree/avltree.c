...
inline struct avl_tree_node *avl_get_parent(const struct avl_tree_node *node) {
...
/* Returns true if the specified AVL tree node has been marked with
...
static inline struct avl_tree_node *
...
static inline struct avl_tree_node *avl_tree_first_or_last_in_order(const struct avl_tree_node *root, int sign) {
    const struct avl_tree_node *first = root;

    if (first)
        while (avl_get_child(first, +sign)) first = avl_get_child(first, +sign);
 ...
 * }
 */
inline struct avl_tree_node *avl_tree_insert(
